	...
	*/
	...
	switch (typeno) {
		...
		case 5: /*  rawbits pgm type */
			...
			c = 0;
			for (i = 0; i < (*height); i++)
				for (j = 0; j < (*width); j++) p[i][j] = data[c++];
...
int main(int argc, char** argv) {
	...
	for (i = 0; i < height; i++)
		for (j = 0; j < width; j++) dp[i * width + j] = imptr[i][j];

	// ROTINA CRONOMETRADA
	T1 = clock();
	for (n = 0; n < 500; n++) bin_img_asm(dp, width, height);
	T2 = clock();
	printf("A sua rotina executou em %6.3f segundos.\n",
		   (((double)(T2 - T1)) / CLOCKS_PER_SEC));

	for (i = 0; i < height; i++)
		for (j = 0; j < width; j++) imptr[i][j] = dp[i * width + j];
